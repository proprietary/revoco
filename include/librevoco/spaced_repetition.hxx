 ...
 * where:
 * EF' - new value of the
 * E-Factor, EF - old value of the E-Factor, q - quality of the
 * response in the 0-5 grade scale.
 * 
 * 6. If EF is less than 1.3 then let EF be 1.3.
 * 
 * 7. If the quality response was lower than 3 then start repetitions
 * for the item from the beginning without changing the E-Factor
 * (i.e. use intervals I(1), I(2) etc. as if the item was memorized
 * anew).  After each repetition session of a given day repeat again
 * all items that scored below four in the quality
 * assessment. Continue the repetitions until all of these items score
 * at least four.
 * 
 */
...
using easiness_factor_t = double;

uint32_t inter_repetition_interval(uint32_t n_days, easiness_factor_t ef) noexcept;

easiness_factor_t update_easiness_factor(easiness_factor_t ef) noexcept;
